...
| Condition (ADR-007 reference) | Evidence | Owner | Status |
|---|---|---|---|
| G9: Data Activator + Teams alert fires on simulated failure | Screenshot/Teams message log | @data-platform-engineer | ☐ Pending — **blocked, root cause fully diagnosed**: this Fabric trial tenant is MSA/personal-signup-rooted; Power Platform (BAP) blocks OAuth for first-party Microsoft services (incl. Teams) on such tenants as a platform policy, not a Global-Admin-toggleable setting. Confirmed via 3 independent dead ends (classic Connectors retired product-wide, Workflows app absent, Power Automate portal OAuth explicitly rejected with `"Microsoft Accounts are not allowed by their BAP administrator"`). This also fully explains J-011's original failure. **Owner explicitly declined a Slack substitute** (hard veto, FB4/@scope-guardian) — will resolve the tenant/Teams situation out-of-band. Parked, no further attempts pending that. See `MIGRATION_JOURNEY.md` J-023. |
