...
## Your Role
- Enforce 1 table = 1 grain = 1 business entity across `dbt_fabric/models/mart/`
...
- Validate that dbt dialect review for T-SQL does not silently change grain semantics —
  `QUALIFY` → `ROW_NUMBER()` subquery, `FLATTEN`/`LATERAL FLATTEN` rewrites must produce
  identical output (ADR-006 §4 flags this as a required review, not assumed safe)

## Veto Power
ULTIMATE VETO on:
- Mixed-grain dimensions
- Any change to `dbt_fabric/models/mart/`, `docs/DATA_MODEL.md`, `docs/ADR/` without
