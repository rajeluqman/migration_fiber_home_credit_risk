...
This repo is governed. Before you edit a Fabric Spark notebook, a dbt-fabric mart model, a
snapshot, or a Data Factory pipeline — or before you "proceed" past an identity/grain question —
you MUST:
...
| Bronze/Silver/Gold storage | OneLake Lakehouse (Delta, native ACID/time-travel) | — | one storage layer, zero cross-cloud egress |
...
| Gold/marts | Fabric Warehouse (T-SQL) | dbt Core + `dbt-fabric` adapter, `dbt_fabric/models/{staging,intermediate,mart}` | Kimball star, SCD2 snapshot — dbt is a NAMED EXCEPTION (ADR-006 §4) |
...
(FB2), no Airflow (FB3), no Slack SDK (FB4). dbt profile adapter must be `type: fabric` (FB5).
...
**Veto holders**: @data-architect (grain/model) · @scope-guardian (stack/scope, FB1-FB6).
**Build**: @senior-data-engineer (dbt-fabric+notebooks+Data Factory) · @data-quality-steward
