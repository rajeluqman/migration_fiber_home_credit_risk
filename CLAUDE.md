...
| Silver transform | OneLake Delta | **Fabric Spark Notebook** (Runtime 1.3 = Spark 3.5), 5 notebooks in `notebooks/` | PII mask (DI-002), XNA→NULL, dedup, native Delta MERGE. ⚠️ On the FTL4 Trial capacity notebooks MUST run against the custom **Small, autoscale-off, 1-fixed-node** `SmallFixedPool` (workspace default) — NOT the oversized Starter Pool, which fails every job at Livy admission with HTTP 430 (ADR-012, J-019) |
